    ...
    const authenticateUser = async function(username, password, done) {
        ...
        try {
            ...
            winston_logger.error(
                `authenticateUser: Getting user ${username} failed. Error: %o`,
                err
            );
            done(err);
        }
        if (!user) {
            winston_logger.debug(
                `authenticateUser: No user with username ${username}. Authentication failed.`
            );
        ...
        try {
            winston_logger.debug(
                `authenticateUser: Checking password for user ${username}.`
            );
            if (await bcrypt.compare(password, user.password)) {
                winston_logger.debug(
                    `authenticateUser: Successfully authenticated user ${username}.`
                );
                return done(null, user);
            } else {
                winston_logger.debug(
                    `authenticateUser: Authentcation of user ${username} failed. ` +
            ...
            winston_logger.error(
                `authenticateUser: Error when comparing hashes for user ${username}. ` +
                    "Error: %o",
                err
            );
            done(err);
