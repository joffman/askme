...
router.get("/", async function(req, res) {
    try {
		...
		console.log("Got request for collection-ratings with collection-id:", collectionId);
...
router.post("/", async function(req, res) {
    ...
    try {
		var findResult = await database.findCollectionRating(collectionId, userId);
		console.log("Result of findRating:", findResult);
		if (findResult.found) {
			console.log("Rating already exists. Updating rating...");
			var ret = await database.updateCollectionRating(rating, collectionId, userId);
			console.log("Return value:", ret);
		} else {
			console.log("Rating does not exist. Creating new rating...");
			var ratingId = await database.addCollectionRating(rating, collectionId, userId);
